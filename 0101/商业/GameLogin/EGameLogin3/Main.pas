...
uses
  ...
  IdAntiFreeze, WinInet, WinHTTP, jpeg, EDcodeUnit, Reg, IdAntiFreezeBase,
  IdBaseComponent, IdTCPConnection, IdTCPClient, StdCtrls, RzButton,
  OleCtrls;
type
...
begin
  ...
  if MyRecInfo.GameListURL <> '' then begin
    ...
    GameListURL := LoginMainImagesB(str,SetDate(DecodeString(s_s03)));//DecodeString_3des(MyRecInfo.GameListURL, CertKey('9òz?L£×Å®'));
    str := LoginMainImagesD(MyRecInfo.BakGameListURL,SetDate(DecodeString(s_s03)));
    BakGameListURL := LoginMainImagesB(str,SetDate(DecodeString(s_s03)));//DecodeString_3des(MyRecInfo.BakGameListURL, CertKey('9òz?L£×Å®'));
    str := LoginMainImagesD(MyRecInfo.PatchListURL,SetDate(DecodeString(s_s03)));
    PatchListURL := LoginMainImagesB(str,SetDate(DecodeString(s_s03)));//DecodeString_3des(MyRecInfo.PatchListURL, CertKey('9òz?L£×Å®'));
    g_boGameMon := MyRecInfo.boGameMon;
    str := LoginMainImagesD(MyRecInfo.GameMonListURL,SetDate(DecodeString(s_s03)));
    GameMonListURL := LoginMainImagesB(str,SetDate(DecodeString(s_s03)));//DecodeString_3des(MyRecInfo.GameMonListURL, CertKey('9òz?L£×Å®'));
    str := LoginMainImagesD(MyRecInfo.GameESystemUrl,SetDate(DecodeString(s_s03)));
    GameESystemURL := LoginMainImagesB(str,SetDate(DecodeString(s_s03)));//DecodeString_3des(MyRecInfo.GameESystemUrl, CertKey('9òz?L£×Å®'));
    ClientFileName := MyRecInfo.ClientFileName;
    str := LoginMainImagesD(MyRecInfo.GatePass,SetDate(DecodeString(s_s03)));
    g_sGatePassWord := LoginMainImagesB(str,SetDate(DecodeString(s_s03)));//DecodeString_3des(MyRecInfo.GatePass, CertKey('9òz?L£×Å®'));
