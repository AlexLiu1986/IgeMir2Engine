...
begin
   ...
   end else  FrmMain.BorderStyle := bsSingle;
   LoadWMImagesLib(nil);
...
//×Ô¶¯·ÅÒ©
      ...
      if ((g_EatingItem.s.StdMode = 0) or (g_EatingItem.s.StdMode = 3)) then begin  //¼ì²éÒ©Æ·ÊýÁ¿ ²»¹»Ôò½â°ü
     ...
     if ((g_EatingItem.s.StdMode = 0) or (g_EatingItem.s.StdMode = 3)) and autoop then begin   //²éÑ¯½â°ü´úÂë
   ...
   if (i = MAXBAGITEMCL) or (bCount = 0) then  begin   //²éÑ¯¿ÕÎ»
     ...
     if ((g_EatingItem.s.StdMode = 0) or (g_EatingItem.s.StdMode = 3)) and (ACount > 5) and  (bCount = 0) and g_AutoPut then  begin
...
var
   ...
   str4: string; //ÈËÎï£º¾Æ2Ïà¹ØÊôÐÔ½ÓÊÕÓÃµÄ±äÁ¿
   str5: string; //Ó¢ÐÛ£º¾Æ2Ïà¹ØÊôÐÔ½ÓÊÕÓÃµÄ±äÁ¿
   sLoginKey: string;
   d: TDirectDrawSurface;
begin
...
//-----------------------------------------------------------
      ...
      SM_FIRDRAGONPOINT: begin     //Ó¢ÐÛÅ­ÆøÖµ
        ...
        if g_HeroItems[U_BUJUK].s.Shape=9 then
         ...
         begin
            ...
            if msg.Ident = SM_MOVEMESSAGE then begin
               case msg.Series of
                ...
                1: Dscreen.AddCenterLetter(Lobyte(Msg.Param),Hibyte(msg.Param),msg.Tag,str);
               end;
            end else
...
begin
  {$if Version <> 0}
  if FindWindow('TFrmMain','´«ÆæµÇÂ½Æ÷ bY 56M2'){²éÕÒÊÇ·ñÓÐ´ËÀàµÄ´°Ìå} = 0 then
  begin{²»Îª0Ôò³ÌÐòÒÑÔËÐÐ}
   // Close;
  end;
  {$IFEND}
//×Ô¶¯º°»°
...
begin
  try
    ...
    sDir := format('config\Ly%s_%s',[g_sLogoText,CharName]);
