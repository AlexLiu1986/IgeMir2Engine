...
var
  ...
  g_sVersion        :String = '版本号：2008.12.31';
...
var
  ...
  LoadInteger: Integer;
begin
  if sUserName <> '' then sFileName := format(SDOCONFIGFILE,[g_sLogoText,sUserName,'SdoAssistant'])
    else sFileName:=format(CONFIGFILE,['Assistant']);
  if not DirectoryExists('config') then  CreateDir('config');
  if not DirectoryExists(format('config\Ly%s_%s',[g_sLogoText,sUserName])) then
    CreateDir(format('config\Ly%s_%s',[g_sLogoText,sUserName]));
