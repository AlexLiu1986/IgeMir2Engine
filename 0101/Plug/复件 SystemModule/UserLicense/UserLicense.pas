...
Try
        ...
        2: begin//使用天数
            if ReadRegKey(1, FSubKeyPath, FUserInfo, sRegisterCodeOfDay) then begin
              if IniReadString(FUserInfo, sRegisterInfo) and CompareLStr(sRegisterCodeOfDay, sRegisterInfo, Length(sRegisterCodeOfDay)) then begin
                ...
                {$IF UserMode1 = 1}
                if Int(FRegisterCodeOfDay.MaxDate) < Int(StrToDate(DecodeInfo(s108))) then begin//如果插件设置的限期比文件里的大,则修改日期 20080701
  ...
  UpDate(FLicenseInfo);
    ...
    MainOutMessasge('[异常] SystemModule:TLicense.Create',0);
  end;
end;
