...
interface
uses
  Windows, Classes, SysUtils, EngineAPI, EngineType, PlugShare;
...
Try
 // nPower := 0;
  ...
  if Magic <> nil then begin
            ...
            if TMagicManager_CheckAmulet(PlayObject, 1, 1, nAmuletIdx) then begin
              ...
              case MagicConfig.nSelMagicID of //
                ...
                SKILL_HANGMAJINBUB {14}: begin //幽灵盾 00494277
                    ...
                    if TBaseObject_MagMakeDefenceArea(PlayObject, nTargetX, nTargetY, 3, nPower, 1) > 0 then
                      Result := True;
                  end;
                SKILL_DEJIWONHO {15}: begin //神圣战甲术 004942E5
                    Ability := TBaseObject_WAbility(PlayObject)^;
                    nPower := TBaseObject_GetAttackPower(PlayObject, GetPower13(60) + LoWord(Ability.nSC) * 10, SmallInt(HiWord(Ability.nSC) - LoWord(Ability.nSC)) + 1);
                    if TBaseObject_MagMakeDefenceArea(PlayObject, nTargetX, nTargetY, 3, nPower, 0) > 0 then
                      Result := True;
                  end;
    ...
    MainOutMessage('[异常] MagicManage.DoSpell');
  end;
end;
...
begin
  ...
  if g_MagicList.Count > 0 then begin//20080629
    ...
    MainOutMessage('[异常] MagicManage.GetUserMagic');
  end;
end;
...
begin
  ...
  if g_MagicList.Count > 0 then begin//20080629
    ...
    MainOutMessage('[异常] MagicManage.GetMagicConfig');
  end;
end;
...
begin
  ...
  if g_MagicList.Count > 0 then begin//20080629
    ...
    MainOutMessage('[异常] MagicManage.LoadMagicListToEngine');
  end;
end;
...
begin
  ...
  if m_nFileHandle > 0 then Result := True
    ...
    MainOutMessage('[异常] MagicManage.OpenMagicFile');
  end;
end;
...
begin
  ...
  if OpenMagicFile() then begin
    ...
    if FileRead(m_nFileHandle, DBHeader, SizeOf(TDBHeader)) = SizeOf(TDBHeader) then begin
      //n4ADB04 := DBHeader.nMagicCount;//20080523
      if DBHeader.nMagicCount > 0 then begin//20080629
    ...
    MainOutMessage('[异常] MagicManage.UpData');
  end;
end;
...
begin
  ...
  if OpenMagicFile() then begin
    ...
    MainOutMessage('[异常] MagicManage.AddData');
  end;
end;
...
begin
  ...
  if OpenMagicFile() then begin
    ...
    MainOutMessage('[异常] MagicManage.DelData');
  end;
end;
...
begin
  ...
  if OpenMagicFile() then begin
    ...
    MainOutMessage('[异常] MagicManage.LoadMagicList');
  end;
end;
...
begin
  ...
  g_MagicList := nil;
    ...
    MainOutMessage('[异常] MagicManage.UnLoadMagicList');
  end;
end;
