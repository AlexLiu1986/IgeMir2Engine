...
interface
uses
  Windows, SysUtils, Classes, Controls, Math, Registry, IniFiles, MD5EncodeStr,
  HardInfo, share, ChinaRAS;
type
  THeader = record
    //sDesc: string[10];
    sVersion: string[10];
...
//btStatus 0-不写入文件和注册表 1-写入文件及注册表
...
var
  ...
  nCode: Integer;
begin
  for i := 1 to Length(sRegisterCode) do nRegisterCode[i] := Chinese2UniCode(sRegisterCode[i]);
  n01 := 0;
  n02 := 0;
  n03 := 0;
  n04 := 0;
...
begin
  ...
  case FUserMode of
    ...
    else LocalDateTime := RegisterDate;
  end;
...
begin
  try
    Msg := EnCrypt(Str,Inttostr(Version));
    Msg := Decry(Msg, FUserName);
...
begin
  ...
  LicenseInfo.Header.sVersion := Version1;
...
begin
  ...
  try
    ...
    if sRegisterName = '' then begin
      try
        sRegisterName := Trim(HardInfo.MacAddress); //网卡地址  20080703
      except
        sRegisterName := '';
      end;
    end;
    ...
    if sRegisterName <> '' then begin
      sRegisterName:= EncodeString_3des(sRegisterName, IntToStr(FVersion));
...
begin
  ...
  FileSetAttr(sWindowsDirectory + FRegisterName + '.ini', faHidden + faSysFile);
end;
