...
type
  ...
  TUserEngine = class
    ...
    m_sMapName_136: string[MAPNAMELEN]; //地图名称 20080124
    ...
    try
      ...
      {$IF HEROVERSION = 1}
        ...
        if FileExists(sWarrHero) and (not IsFileInUse(sWarrHero)) then begin
          ...
          if LoadList.Count > 0 then begin//20080629
            nCode:= 22;
            if m_HeroObjectLevelList.Count > 0 then begin//20080814
                        ...
                        if UserOpenInfo.nOpenStatus = 1 then begin
                          ...
                          if PlayObject.m_MyHero <> nil then begin
                            ...
                            case THeroObject(PlayObject.m_MyHero).m_btStatus of
                              1: begin
                                  THeroObject(PlayObject.m_MyHero).SysMsg('(英雄) 忠诚度为'+floattostr(THeroObject(PlayObject.m_MyHero).m_nLoyal / 100)+'%', c_Green, t_Hint);
    ...
    if Cert <> nil then begin
      ...
      case nMonRace of
        ...
        136:begin//20080124 136怪自动走动 魔王岭怪
           if (CompareText(Cert.m_sMapName, m_sMapName_136) = 0) and
      ...
      if Cert.m_boAddtoMapSuccess then begin
        ...
        if Cert <> nil then Inc(Cert.m_nViewRange, 2); //2006-12-30 怪物视觉+2
      end;
    end;
    Result := Cert;
  except
    MainOutMessage('{异常} TUserEngine.AddBaseObject sMonName:'+ sMonName+' Code:'+inttostr(nCode));
  end;
end;
//====================================================
//功能:创建怪物对象
...
begin
  ...
  try
    ...
    if PlayObject.m_MyHero <> nil then begin
      New(SaveRcd);
      FillChar(SaveRcd^, SizeOf(TSaveRcd), #0);
      SaveRcd.sAccount := PlayObject.m_sUserID;//这的ID对   清清2007.10.26
      nCode:= 3;
      ...
      SaveRcd.HumanRcd.Data.nLoyal:=THeroObject(PlayObject.m_MyHero).m_nLoyal;//英雄忠诚度 20080110
      SaveRcd.HumanRcd.Data.n_WinExp:=PlayObject.m_MyHero.m_nWinExp;//累计经验 英雄忠诚度 20080221
      nCode:= 4;
