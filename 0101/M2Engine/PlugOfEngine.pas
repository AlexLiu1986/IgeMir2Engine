...
//function TBaseObject_MagProtectionDefenceUp(BaseObject: TBaseObject; nLevel: Integer): Boolean; stdcall;//护体神盾 20080107
...
//function TEnvirnoment_boCANRIDE(Envir: TEnvirnoment): PBoolean; stdcall;//未使用 20080329
//function TEnvirnoment_boCANBAT(Envir: TEnvirnoment): PBoolean; stdcall;//未使用 20080329
...
function TPlugOfEngine_GetUserName(): Pchar; stdcall;//输出M2标题，判断是否IGEM2 20081203
...
begin
  ...
  if TPlayObject(BaseObject).m_StorageItemList.Count < 46 then begin//20080416
    New(UserItem);
    UserItem.MakeIndex := AddItem.MakeIndex;
    UserItem.wIndex := AddItem.wIndex;
    UserItem.Dura := AddItem.Dura;
    ...
    TPlayObject(BaseObject).m_StorageItemList.Add(UserItem);
    Result := True;
    //DisPose(UserItem);
  end;
end;
...
{//护体神盾 20080107
function TBaseObject_MagProtectionDefenceUp(BaseObject: TBaseObject; nLevel: Integer): Boolean;
begin
  Result :=BaseObject.MagProtectionDefenceUp(nLevel);
end;}
...
//20080302 灵符接口
function TPlayObject_nGameGird(BaseOjbect: TPlayObject): PInteger;
begin
  Result := @BaseOjbect.m_nGAMEGIRD;
end;
...
//物品掉落提示规则 20080226
procedure TPlayObject_SetCheckClientDropHint(ActionDropItem: TObjectActionItem1);
begin
  zPlugOfEngine.CheckCanDropHint := ActionDropItem;
end;
//开启宝箱提示规则 20080226
procedure TPlayObject_SetCheckClientOpenBoxsHint(ActionDropItem: TObjectActionItem1);
begin
  zPlugOfEngine.CheckCanOpenBoxsHint := ActionDropItem;
end;
//物品永不暴出规则  20080226
procedure TPlayObject_SetCheckClientNoDropItem(ActionDropItem: TObjectActionItem);
begin
  zPlugOfEngine.CheckCanNoDropItem := ActionDropItem;
end;
//物品规则里的 挖取提示规则  20080226
procedure TPlayObject_SetCheckClientButchHint(ActionDropItem: TObjectActionItem1);
begin
  zPlugOfEngine.CheckCanButchHint:= ActionDropItem;
end;
//物品规则里的 禁止英雄使用规则 20080419
procedure TPlayObject_SetCheckClientHeroUseItem(ActionDropItem: TObjectActionItem);
begin
  zPlugOfEngine.CheckCanHeroUseItem:= ActionDropItem;
end;
//物品规则里的 禁止捡起(除GM外) 20080611
procedure TPlayObject_SetCheckClientPickUpItem(ActionDropItem: TObjectActionItem);
begin
  zPlugOfEngine.CheckCanPickUpItem:= ActionDropItem;
end;
//物品规则 死亡掉落 20080614
procedure TPlayObject_SetCheckClientDieDropItems(ActionDropItem: TObjectActionItem);
begin
  zPlugOfEngine.CheckCanDieDropItems:= ActionDropItem;
end;
...
//输出M2标题，判断是否IGEM2 20081203
function TPlugOfEngine_GetUserName(): Pchar;
begin
  Result := PChar(FrmMain.Caption);
end;
