...
begin
  PageControl1.ActivePage:=TabSheet1;//20080901 增加
  RefHumanInfo();
...
begin
  ...
  if PlayObject.m_boTrainingNG then begin
    PlayObject.m_NGLevel := EditNGLevel.Value;//20081005 内功等级
...
{$IF HEROVERSION = 1}
  if PlayObject.m_MyHero <> nil then begin
    ...
    THeroObject(PlayObject.m_MyHero).m_nLoyal:= nLoyal;//英雄的忠诚度(20080110)
    if THeroObject(PlayObject.m_MyHero).m_boTrainingNG then begin
      THeroObject(PlayObject.m_MyHero).m_NGLevel := EditHeroNGLevel.Value;//20081005 内功等级
      THeroObject(PlayObject.m_MyHero).m_ExpSkill69 := EditHeroExpSkill69.Value;//20081005 内功心法当前经验
