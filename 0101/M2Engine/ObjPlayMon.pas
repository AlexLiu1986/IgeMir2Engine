...
interface
uses
  Windows, SysUtils, Classes, Grobal2, ObjBase, IniFiles, ObjHero{20080208 增加};
type
  TPlayMonster = class(TBaseObject)
    m_dwThinkTick: LongWord;
    m_boDupMode: Boolean; //人物重叠了
    //m_nNotProcessCount: Integer; //未被处理次数，用于怪物处理循环  未使用 20080329
    m_nTargetX: Integer;
    m_nTargetY: Integer;
    ...
    m_Skill_75_Tick: LongWord; //护体神盾使用间隔  20080107
    ...
    wSkill_05: Word; //魔法盾
    wSkill_66: Word; //四级魔法盾 20080728
    wSkill_01: Word; //雷电术
    wSkill_02: Word; //地狱雷光
    wSkill_03: Word; //冰咆哮
    wSKILL_58: Word;//流星火雨 20080528
    wSKILL_36: Word;//火焰冰 20080410
    wSKILL_45: Word;//灭天火 20080410
    wSkill_04: Word; //火龙焰
...
begin
  ...
  m_Skill_75_Tick := GetTickCount(); //护体神盾使用魔法间隔 20080107
...
begin
  ...
  case m_btJob of
    0: begin
        ...
        wSkill_75 := CheckUserMagic(SKILL_75); //护体神盾 20080107
      end;
    1: begin
        ...
        wSkill_66:= CheckUserMagic(SKILL_66); //四级魔法盾 20080728
        wSKILL_58 := CheckUserMagic(SKILL_58);//流星火雨 20080528
        wSKILL_36 := CheckUserMagic(SKILL_MABE);//火焰冰 20080410
        wSKILL_45 := CheckUserMagic(SKILL_45);//灭天火 20080410
        wSkill_75 := CheckUserMagic(SKILL_75); //护体神盾 20080107
    ...
    2: begin
        ...
        wSKILL_59 := CheckUserMagic(SKILL_59);//噬血术 20080528
...
begin
  ...
  try
    if m_btJob > 0 then begin
      ...
      if m_btJob = 2 then begin
        nCode:= 1;
        if (wSkill_06 > 0) or (wSkill_10 > 0) then begin//施毒术
          nCode:= 2;
        ...
        if (wSkill_07 > 0) or (wSkill_59 > 0) then begin //灵魂火符  噬血术
          nCode:= 8;
    ...
    on E: Exception do begin
      MainOutMessage('{异常} TPlayMonster.CheckDoSpellMagic Code:'+Inttostr(nCode));
    end;
  end;
end;
...
begin
  ...
  try
    ...
    nCode:= 2;
    if SearchPickUpItemOK() then SearchPickUpItem(500); //捡物品
    nCode:= 3;
    EatMedicine(); {吃药}
    nCode:= 4;
    ...
    on E: Exception do begin
      MainOutMessage('{异常} TPlayMonster.Think Code:'+Inttostr(nCode));
    end;
  end;
end;
//是否可以捡起物品
...
//  sName: string;
resourcestring
  sExceptionMsg2 = '{异常} TPlayMonster::SearchItemRange 1-%d %s %s %d %d %d';
begin
  try
    nCode:= 0;
    ...
    MainOutMessage('{异常} TPlayMonster.SearchPickUpItem Code:'+inttostr(nCode));
  end;
end;
//是不是可以捡的药品
  ...
  begin
    ...
    case wMagIdx of
      ...
      SKILL_75 {75}: begin //护体神盾 20080107
          if GetTickCount()- m_boProtectionTick < g_Config.dwProtectionTick then Exit; //护体神盾使用间隔 20080109
...
function TPlayMonster.WarrorAttackTarget(): Boolean;
  ...
  begin
    ...
    if ((abs(m_TargetCret.m_nCurrX - m_nCurrX) > 1{2}) and (abs(m_TargetCret.m_nCurrX - m_nCurrX) < 5)) or
    ...
    if m_boUseThrusting then DoSpellMagic(SKILL_ERGUM);     //关闭刺杀
    if m_boUseHalfMoon then DoSpellMagic(SKILL_BANWOL);     //关闭半月
    ...
    if (abs(m_TargetCret.m_nCurrX - m_nCurrX) <= 2) and (abs(m_TargetCret.m_nCurrY - m_nCurrY) <= 2) then begin//目标近身时
      ...
      case Random({5}4) of//20080619
       ...
       { 1:begin                                                                  20080619 注释
           if (wSKILL_43 > 0) then DoSpellMagic(SKILL_43)             //打开龙影
           else if (wSKILL_74 > 0) then DoSpellMagic(SKILL_74)    //打开逐日剑法 20080528
           else if (wSkill_13 > 0) then DoSpellMagic(SKILL_BANWOL)    //打开半月
           else if (wSKILL_40 > 0) then DoSpellMagic(SKILL_40)        //抱月刀法
           else if (wSkill_11 > 0) then DoSpellMagic(SKILL_FIRESWORD);//使用烈火
        end;  }
        1:begin
           if (wSKILL_74 > 0) then begin
             DoSpellMagic(SKILL_74);//打开逐日剑法 20080528
  ...
  try
    ...
    if m_WAbil.MP > 0 then begin
      ...
      if (wSkill_75 > 0) and (m_wStatusTimeArr[STATE_ProtectionDEFENCE] = 0) and (not m_boProtectionDefence) and
        (GetTickCount - m_boProtectionTick > g_Config.dwProtectionTick) then DoSpellMagic(wSkill_75);//护体神盾 20080405
      ...
      else if m_boUseHalfMoon then m_wHitMode := 5 //使用半月
    ...
    //if m_WAbil.HP <= Round(m_WAbil.MaxHP * 0.25) then m_boAutoAvoid:= True;//是否能躲避 20080715 20081217注释
  except
  end;
end;
//检查使用的魔法
...
function TPlayMonster.WizardAttackTarget(): Boolean; {法师攻击}
  ...
  begin
    ...
    and (GetTickCount - m_boProtectionTick > g_Config.dwProtectionTick) then begin {使用 护体神盾 20080107}
      Result := wSkill_75;
      Exit;
    end;
    ...
    if CheckTargetXYCount(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, 3) > 1 then begin//取指定坐标范围内的目标数量
      if (abs(m_TargetCret.m_nCurrX - m_nCurrX) <= 1) and (abs(m_TargetCret.m_nCurrY - m_nCurrY) <= 1) then begin
        if (m_Master <> nil) and (wSkill_02 > 0) then begin//20080803 人形看怪使用地狱雷光
...
try
  ...
  m_dwHitTick := GetTickCount();//20080715 增加
    ...
    MainOutMessage('{异常} TPlayMonster.WizardAttackTarget:'+inttostr(nCode));
  end;
end;
...
function TPlayMonster.TaoistAttackTarget(): Boolean; {道士攻击}
    ...
    begin
      ...
      if CheckUserItemType(1,0) or (GetUserItemList(1,0) >= 0) then begin
        case Random(2) of
          ...
          1: begin
             if wSkill_59 > 0 then Result := wSkill_59//噬血术
  ...
  begin
    ...
    try
      ...
      if (wSkill_73 > 0) and (m_wStatusTimeArr[STATE_BUBBLEDEFENCEUP ] = 0) and (wSkill_75 = 0) then begin {使用 道力盾 20080405}
        Result := wSkill_73;
        Exit;
      end;
...
try
    ...
    if nMagicID > 0 then begin
      ...
      case nMagicID of
       { SKILL_FIRECHARM, SKILL_DEJIWONHO: begin //道用符的技能,装备里没有,直接用包裹里的物品,不用自动换物品 20080415
        ...
        SKILL_AMYOUNSUL, SKILL_GROUPAMYOUNSUL: begin //施毒术
  ...
  m_dwHitTick := GetTickCount();//20080715 增加
    ...
    MainOutMessage('{异常} TPlayMonster.TaoistAttackTarget Code:'+inttostr(nCode));
  end;
end;
...
var
  ...
  nCheckCode: Integer;
resourcestring
  sExceptionMsg = '{异常} TPlayMonster.Run Code:%d';
begin
  nCheckCode:= 0;
  try
    if (not m_boGhost) and (not m_boDeath) and (not m_boFixedHideMode) and (not m_boStoneMode) and
      ...
      if (GetTickCount - m_dwSearchEnemyTick) > 1000 then begin
        m_dwSearchEnemyTick := GetTickCount();
        //if (m_TargetCret = Self) then m_TargetCret := nil; //20080410
        if m_TargetCret = nil then SearchTarget(); {搜索目标}
      end;
      nCheckCode:= 3;
      ...
      if not m_boWalkWaitLocked and ({Integer}(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin//20080715
        ...
        if not m_boRunAwayMode then begin
          if not m_boNoAttackMode then begin
            if (m_TargetCret <> nil) and (not m_TargetCret.m_boDeath) and (m_TargetCret.m_WAbil.HP > 0) then begin  //20080424
              nCheckCode:= 51;
...
//  MagicInfo: pTMagic;
  UserMagic: pTUserMagic;
  StdItem: pTStdItem;
  StdItemNameArray: array[0..13] of string[16];//20080416 扩展到13 支持斗笠
begin
    ...
    if FileExists(sFileName) then begin
      ...
      if ItemIni <> nil then begin
       ...
       if sCopyHumBagItems <> '' then begin
        ...
        StdItemNameArray[U_ZHULI] := ItemIni.ReadString('UseItems','UseItems13' {'CHARMNAME'}, ''); // '斗笠'; 20080416
        for I := U_DRESS to U_ZHULI do begin //20080416 斗笠
...
begin
  ...
  if nSpellPoint > 0 then begin
    ...
    m_wStatusArrValue[2] := Round(MakeLong(LoWord(m_WAbil.SC), HiWord(m_WAbil.SC))*(UserMagic.btLevel * 0.2 + 0.4));//提升值 20080826
    RecalcAbilitys();
    Result := True;
  end;
end;
