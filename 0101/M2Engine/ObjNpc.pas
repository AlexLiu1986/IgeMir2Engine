...
type
  ...
  private
    ...
    procedure ActionOfWAPMOVETOTIME(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);//雪域地图传送，时间到传送回城点 20081230
  ...
  if Guild <> nil then begin
    if (PlayHum <> nil) and (not PlayHum.m_boNotOnlineAddExp){ and (PlayHum.m_boAllowGuild) }then begin //人物存在,不是离线挂机,允许加入行会
      if not Guild.IsMember(UserName) then begin //不是行会成员
        if (PlayHum.m_MyGuild = nil) and (Guild.m_RankList.Count < 400) then begin//人物没有行会,并且要加入的行会人员没有满
...
//功给：雪域地图传送，时间到传送回城点 20081230
//格式：WAPMOVETOTIME 地图名 X坐标 Y坐标 时间(秒) 是否组队进入(0/1)
procedure TNormNpc.ActionOfWAPMOVETOTIME(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  nX, nY, nTime, II: Integer;
  sMap: string;
  Envir: TEnvirnoment;
  boGroupMove: Boolean;
begin
  try
    ...
    nTime:= Str_ToInt(GetLineVariableText(PlayObject,QuestActionInfo.sParam4), -1);//时间(秒)
    boGroupMove:= Str_ToInt(GetLineVariableText(PlayObject,QuestActionInfo.sParam5), 0) <> 0;//是否组队进入
    if (sMap = '') or (nX < 0) or (nY < 0) or (nTime < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_WAPMOVETOTIME);
      Exit;
    end;

    Envir := g_MapManager.FindMap(sMap);//查找地图场景
    if Envir <> nil then begin//判断地图是否存在
      if (PlayObject.m_GroupOwner <> nil) and boGroupMove then begin//有组队
        if (PlayObject = PlayObject.m_GroupOwner) then begin//是队长
            ...
            for II := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
              if (not TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[II]).m_boDeath) and
                (not TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[II]).m_boGhost) then begin
                  TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[II]).SpaceMove(sMap, nX, nY, 0);
                  if TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[II]).m_PEnvir.sMapName = sMap then begin//地图是否传送成功
                    TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[II]).m_dwUserTick[0]:= nTime;
                    //发总的时间给客户端
                  end;
              end;
            end;//for
          ...
          PlayObject.SendRefMsg(RM_SPACEMOVE_FIRE, 0, 0, 0, 0, '');
          PlayObject.SpaceMove(sMap, nX, nY, 0);
          if PlayObject.m_PEnvir.sMapName = sMap then begin//地图是否传送成功
    ...
    MainOutMessage('{异常} TNormNpc.ActionOfWAPMOVETOTIME');
  end;
end;
//改变发型
...
begin
  QuestActionInfo.sParam1 := GetLineVariableText(PlayObject, QuestActionInfo.sParam1);//20080602 路径支持变量
  sListFileName := g_Config.sEnvirDir + m_sPath + QuestActionInfo.sParam1;
  LoadList := TStringList.Create;
  try
    {if FileExists(sListFileName) then begin
...
try
 ...
 // nGameGird := Str_ToInt(QuestActionInfo.sParam2, -1);//20080315
  ...
  case cMethod of
    ...
    '-': begin
        //nGameGird := _MAX(0, PlayObject.m_nGameGird - nGameGird);
        //PlayObject.m_nGameGird := nGameGird;
...
try
  ...
  if PlayObject.m_btRaceServer = RC_HEROOBJECT then begin
     ...
     PlayObject.CompareSuitItem(False);//200080729 套装
     THeroObject(PlayObject).SendMsg(m_Master, RM_HEROABILITY, 0, 0, 0, 0, ''); //发送英雄属性
  end else begin
     PlayObject.RecalcAbilitys();
     PlayObject.CompareSuitItem(False);//200080729 套装
...
var
  LoadList: TStringList;
  sFileName, MonName, sValue: string;
...
try
  ...
  if MonName <>'' then begin //传值过程
    if GetValNameValue(QuestActionInfo.sParam2, sValue, nValue, nDataType) then begin
     if not SetValNameValue(QuestActionInfo.sParam2, MonName, nValue, nDataType) then
...
try
  ...
  //nMineCount:= Str_ToInt(QuestConditionInfo.sParam2, -1);//数量
  nMineCount := Str_ToInt(GetLineVariableText(PlayObject,QuestConditionInfo.sParam2),-1);//20080501
  //nDura:= Str_ToInt(QuestConditionInfo.sParam3, -1);//纯度
  ...
  if PlayObject.m_ItemList.Count > 0 then begin//20080628
    for I:=0 to PlayObject.m_ItemList.Count - 1 do begin
      UserItem := PlayObject.m_ItemList.Items[I];
      StdItem := UserEngine.GetStdItem(UserItem.wIndex);
      if (StdItem <> nil) and (StdItem.StdMode = 43) and (CompareText(StdItem.Name, sMineName) = 0) then begin
...
var
      ...
      for I := 0 to ConditionList.Count - 1 do begin
        ...
        case QuestConditionInfo.nCMDCode of
          ...
          nSC_CHECKLISTTEXT:begin //检查文件是否包含指定文本 20080428
              ...
              if QuestConditionInfo.sParam1[1] = '\' then QuestConditionInfo.sParam1 := Copy(QuestConditionInfo.sParam1, 2, Length(QuestConditionInfo.sParam1) - 1);
              if QuestConditionInfo.sParam1[2] = '\' then QuestConditionInfo.sParam1 := Copy(QuestConditionInfo.sParam1, 3, Length(QuestConditionInfo.sParam1) - 2);
              if QuestConditionInfo.sParam1[3] = '\' then QuestConditionInfo.sParam1 := Copy(QuestConditionInfo.sParam1, 4, Length(QuestConditionInfo.sParam1) - 3);
              QuestConditionInfo.sParam1 := GetLineVariableText(PlayObject, QuestConditionInfo.sParam1);//20080602 路径支持变量
              if not CheckTextInList(s01, QuestConditionInfo.sParam1) then Result := False;
            end;
  ...
  //------------------------------------------------------------------------------
          ...
          nSC_WAPMOVETOTIME: ActionOfWAPMOVETOTIME(PlayObject, QuestActionInfo);//雪域地图传送，时间到传送回城点 20081230
  ...
  //-----------------------------------------------------------------------------
          ...
          nSC_GROUPMAPMOVE: begin //组编传送,只有队长才能使用
  ...
  try
    ...
    if CompareText(QuestActionInfo.sParam2, 'HERO') = 0 then begin
      ...
      if PlayObject.m_MyHero <> nil then begin
        ...
        PlayObject.SendMsg(PlayObject, RM_HEROABILITY, 0, 0, 0, 0, ''); //发送英雄属性
...
try
  ...
  if CompareText(QuestActionInfo.sParam4, 'HERO') = 0 then begin
    ...
    if PlayObject.m_MyHero <> nil then begin
      ...
      THeroObject(PlayObject.m_MyHero).SendMsg(PlayObject, RM_HEROABILITY, 0, 0, 0, 0, ''); //发送英雄属性
      THeroObject(PlayObject.m_MyHero).SendMsg(PlayObject, RM_SUBABILITY, 0, 0, 0, 0, '');
    end;   
  end else begin
    case cMethod of
      '+': begin
...
try
  ...
  MainOutMessage('{异常} TNormNpc.ConditionOfCheckMonMapCount');
end;
end;
//-----------------------------------------------------------------------------
//检测地图命令  20080426
