...
begin
  ...
  LoadHumDB(FrmMain.Hum_db3.Text , m_MainHumLists);
  ...
  with FrmMain do begin
    Hum_db3.Enabled := True;
    Mir_db3.Enabled := True;
...
{  Inc(nC, GetFileSize(m_sMainRoot + '\IDDB\ID.DB', 120, Sizeof(TMirID)));
  ...
  Inc(nC, GetFileSize(m_sMainRoot + '\FDB\MIR.DB', Sizeof(TDBFileHeader), Sizeof(THumData)));   }
    ...
    begin
      ...
      Inc(nC, GetFileSize(FrmMain.Hum_db3.text+ '\DBServer\FDB\HUM.db', Sizeof(TDBHeader), Sizeof(TDBHum)));
...
//打开从库数据文件  20071122
  f_HumH  := FileOpen(FrmMain.Hum_db3.text, 0);
  f_MirH  := FileOpen(FrmMain.Mir_db3.text, 0);
////////////////////////////////////////////////////////////////
//20071120增加
...
//////////////////////////////////////////////////////////////////
  ...
  m_nCurMax  := GetFileSize({sRoot + '\DBServer\FDB\Hum.db'}FrmMain.Hum_db3.text, Sizeof(TDBHeader1), Sizeof(TDBHum)); //FileSeek(f_HumH, Sizeof(TDBFileHeader), 2) div Sizeof(DBHum);
  ...
  while FileRead(f_HumH, DBHum, Sizeof(DBHum)) = Sizeof(DBHum) do  //T 循环读出人物数据
    begin
      FileSeek(f_MirH, Sizeof(TDBHeader1)+ Sizeof(THumDataInfo) * m_nCurPostion, 0);
      FileRead(f_MirH, HumData, Sizeof(THumDataInfo));
...
{20071119增加,先更新数据头,再写入数据}
      ...
      NewHumData.Data.sHeroChrName:= HumData.Data.sHeroChrName;//20081231
     ...
     { NewHumData.Data.nExpSkill69: Integer;//内功当前经验 20080930
      NewHumData.Data.nSkill69NH: Integer;//内功当前内力值 20080930
      NewHumData.Data.HumNGMagics: THumNGMagics;//内功技能 20081001
