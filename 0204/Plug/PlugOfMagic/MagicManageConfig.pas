...
uses
  Windows,  SysUtils,  Controls, Forms, Spin, Classes, ComCtrls, StdCtrls, ExtCtrls;
...
type
  ...
  private
    ...
    //procedure DisposeListViewMagic(); //20080523
...
begin
  ...
  ShowModal;
    ...
    MainOutMessage('[异常] MagicManage.Open');
  end;
end;
...
begin
  ...
  if g_MagicList.Count > 0 then begin//20080629
    ...
    MainOutMessage('[异常] MagicManage.RefMagicList');
  end;
end;
...
begin
  ...
  if MagicList.Count > 0 then begin//20080629
    ...
    MainOutMessage('[异常] MagicManage.LoadTUserEngineMagicList');
  end;
end;
...
begin
  ...
  try
    ...
    MainOutMessage('[异常] MagicManage.InMagicList');
  end;
end;
...
begin
  ...
  try
    ...
    MainOutMessage('[异常] MagicManage.GetMagicCountByMagicID');
  end;
end;
...
begin
  ...
  try
    ...
    MainOutMessage('[异常] MagicManage.ButtonAddMagicClick');
  end;
end;
...
begin
  ...
  if (nItemIndex >= 0) and (nItemIndex < g_MagicList.Count) then begin
    ...
    MainOutMessage('[异常] MagicManage.ButtonChgMagicClick');
  end;
end;

procedure TFrmMagicManage.ButtonDelMagicClick(Sender: TObject);
var
//  MagicConfig: pTMagicConfig; //20080523
  ListItem: TListItem;
  nMagicID: Integer;
  nItemIndex: Integer;
  MagicRcd: pTMagicRcd;
...
//    MagicConfig := pTMagicConfig(ListItem.SubItems.Objects[0]);//20080523
    nMagicID := Str_ToInt(ListItem.Caption, 100);
    DelData(nMagicID);
    ListViewMagic.DeleteSelected;
  except
    MainOutMessage('[异常] MagicManage.ButtonDelMagicClick');
  end;
end;
...
begin
  ...
  ButtonLoadMagic.Enabled := True;
    ...
    MainOutMessage('[异常] MagicManage.ButtonLoadMagicClick');
  end;
end;
...
Try
  ...
  LabelSelMagicName.Caption := '名称：' + Magic.sMagicName + #13 + 'ID：' + IntToStr(nSelMagicID);
    ...
    MainOutMessage('[异常] MagicManage.ListBoxMagicClick');
  end;
end;
...
begin
  ...
  for I := 0 to ListBoxMagic.Items.Count - 1 do begin
    ...
    MainOutMessage('[异常] MagicManage.GetMagicOfID');
  end;
end;
...
begin
  try
    ...
    MainOutMessage('[异常] MagicManage.ListViewMagicClick');
  end;
end;
