...
begin
  ...
  try
    if HumDataDB.Open and (HumDataDB.Index(sChrName) = -1) then begin
      ...
      ChrRecord.Header.sName := sChrName;//名称
      ChrRecord.Data.sChrName := sChrName;//名称
...
begin
  ...
  try
    if HumDataDB.Open and (HumDataDB.Index(sChrName) = -1) then begin
      ...
      ChrRecord.Data.btEF := nHeroType;//英雄类型 0-白日门英雄 1-卧龙英雄 20080514
      ChrRecord.Data.nEXPRATE:=100;//杀怪倍数 20071230
      HumDataDB.Add(ChrRecord);
      Result := True;
    end;
          ...
          try
            if HumDataDB.OpenEx then begin
              for i := 0 to ChrList.Count - 1 do begin
                ...
                if HumChrDB.GetBy(QuickID.nIndex, HumRecord) and (not HumRecord.boDeleted) and (not HumRecord.Header.boIsHero) then begin//加不是英雄的条件 20080515
            ...
            try
              if HumDataDB.OpenEx then begin
                for i := 0 to ChrList.Count - 1 do begin
                  ...
                  //如果选择ID不对,则跳过
                  //if QuickID.nIndex <> UserInfo.nSelGateID then Continue;
                  if HumChrDB.GetBy(QuickID.nIndex, HumRecord) and HumRecord.boDeleted and (not HumRecord.Header.boIsHero) then begin//加不是英雄的条件,并且是删除过的角色
                    sChrName := QuickID.sChrName;
                    nIndex := HumDataDB.Index(sChrName);
      ...
      if HumDataDB.count > 0 then begin
        ...
        for I:= 0 to HumDataDB.count -1 do begin
          if HumDataDB.Get(I, HumanRCD) >= 0 then begin
             if (not HumanRCD.Data.boIsHero) then Continue;//继续
             if (HumanRCD.Data.sMasterName='') then Continue;//继续
