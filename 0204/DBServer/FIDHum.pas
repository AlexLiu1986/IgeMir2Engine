...
begin
  Edit1.Text := '';
  Edit2.Text := '';
  IdGrid.Cells[0, 0] := 'µÇÂ¼ÕÊºÅ';
  IdGrid.Cells[1, 0] := 'ÃÜÂë';
  IdGrid.Cells[2, 0] := 'ÓÃ»§Ãû³Æ';
  ...
  ChrGrid.Cells[2, 0] := 'µÇÂ¼ÕÊºÅ';
  ChrGrid.Cells[3, 0] := 'ÊÇ·ñ½ûÓÃ';
  ChrGrid.Cells[4, 0] := '½ûÓÃÊ±¼ä';
  ChrGrid.Cells[5, 0] := '²Ù×÷¼ÆÊý';
  ChrGrid.Cells[6, 0] := 'Ñ¡Ôñ±àºÅ';
  ChrGrid.Cells[7, 0] := 'ÊÇ·ñÓ¢ÐÛ';
end;
...
begin
  ...
  if MessageDlg('ÊÇ·ñÈ·ÈÏÉ¾³ýÈËÎï ' + sChrName + ' £¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
...
begin
  ...
  EdUserId.Text:= ChrGrid.Cells[2, nRow];
end;
...
var
  ...
  nIndex, I: Integer;
  HumRecord: THumInfo;
  HumanRCD,HeroChrRecord: THumDataInfo;//20080220
begin
  try
    sChrName := EdChrName.Text;
    if sChrName = '' then Exit;
    if MessageDlg('ÊÇ·ñÈ·ÈÏ½ûÓÃÈËÎï ' + sChrName + ' £¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
      try
        if HumChrDB.Open then begin
          nIndex := HumChrDB.Index(sChrName);
          if nIndex >= 0 then begin//20090106
  ...
  //--------------------------20080220--------------------------------------------
        if HumDataDB.Open then begin
          ...
          if nIndex >= 0 then begin//20090106
            ...
            if HumanRCD.Data.boHasHero or HumanRCD.Data.boIsHero then begin//ÓÐÓ¢ÐÛ,Ôò°ÑÓ¢ÐÛÒ²½ûÖ¹ 20081005
              if HumDataDB.count > 0 then begin//20090101
                for I:= 0 to HumDataDB.count -1 do begin
                  if HumDataDB.Get(I, HeroChrRecord) >= 0 then begin
                    if (not HeroChrRecord.Data.boIsHero) then Continue;//¼ÌÐø
                    if CompareText(HeroChrRecord.Data.sMasterName, sChrName) = 0 then begin
                      HeroChrRecord.Header.boDeleted:= True;
...
//--------------------------20080220--------------------------------------------
      if HumDataDB.Open then begin
        ...
        if nIndex >= 0 then begin//20090106
          HumDataDB.Get(nIndex, HumanRCD);
          HumanRCD.Header.boDeleted := False;//20080923 ÐÞ¸Ä
          HumDataDB.Update(nIndex, HumanRCD);
          if HumanRCD.Data.boHasHero or HumanRCD.Data.boIsHero then begin//ÓÐÓ¢ÐÛ,Ôò°ÑÓ¢ÐÛÒ²»Ö¸´ 20081005
            if HumDataDB.count > 0 then begin//20090101
              for I:= 0 to HumDataDB.count -1 do begin
                if HumDataDB.Get(I, HeroChrRecord) >= 0 then begin
                  if (not HeroChrRecord.Data.boIsHero) then Continue;//¼ÌÐø
...
begin
  sChrName := EdChrName.Text;
  if sChrName = '' then Exit;
  if MessageDlg('ÊÇ·ñÈ·ÈÏÉ¾³ýÈËÎï ' + sChrName + ' ¼°ÈËÎïÊý¾Ý£¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
        HumChrDB.Delete(sChrName);
      end;
...
begin
  ...
  if MessageDlg('ÊÇ·ñÈ·ÈÏ½ûÓÃ¼ÇÂ¼ ' + IntToStr(nIndex) + ' £¿', mtConfirmation, [mbYes, mbNo], 0) = mrYes then begin
    try
      if HumChrDB.Open then begin
...
begin
  ...
  ChrGrid.Cells[7, nRowCount] := BooleanToStr(HumDBRecord.Header.boIsHero);//20080923
    ...
    if HumDataDB.OpenEx then begin
      ...
      if nIdx >= 0 then begin
        if HumDataDB.Get(nIdx, ChrRecord) >= 0 then begin
          ...
          if CompareText(ChrRecord.Data.sAccount , Trim(EdUserId.Text)) <> 0 then begin
            frmEditRcd.CheckBoxModeAccount.Enabled:= True;
          end;
        end;
      end;
    end;
