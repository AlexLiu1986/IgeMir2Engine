...
//----------------------------英雄先合------------------------------------------
  ...
  while FileRead(f_HumH, DBHum, Sizeof(DBHum)) = Sizeof(DBHum) do begin //T 循环读出人物数据
    ...
    try
      ...
      try
        if HumDataDB1.OpenEx then begin
          ...
          if I >= 0 then begin
            ...
            //m_sCurMessage := '错误(保存英雄失败):'+ DBHum.sChrName + '  数据不匹配(Mir.db找不到相关数据)';
            //{$IF THREADWORK}Synchronize({$IFEND}OutMessage{$IF THREADWORK}){$IFEND};
            //Inc(m_nFailed);
            Inc(m_nTotalPostion);
            Inc(m_nCurPostion);
      ...
      if J < 0 then begin //T
        m_sCurMessage := '错误(保存英雄失败):' + DBHum.sChrName + ' ID.DB无匹配账号:'+DBHum.sAccount;
        {$IF THREADWORK}Synchronize({$IFEND}OutMessage{$IF THREADWORK}){$IFEND};
        Inc(m_nFailed);
        Inc(m_nTotalPostion);
        Inc(m_nCurPostion);
...
//-------------------------合并人物--------------------------------------------
  ...
  while FileRead(f_HumH, DBHum, Sizeof(DBHum)) = Sizeof(DBHum) do begin //T 循环读出人物数据
    ...
    try
      ...
      try
        if HumDataDB1.OpenEx then begin
          ...
          if I >= 0 then begin
            ...
            //m_sCurMessage := '错误(保存人物失败):'+ DBHum.sChrName + '  数据不匹配(Mir.db找不到相关数据)!';
            //{$IF THREADWORK}Synchronize({$IFEND}OutMessage{$IF THREADWORK}){$IFEND};
            //Inc(m_nFailed);
            Inc(m_nTotalPostion);
            Inc(m_nCurPostion);
      ...
      J := m_SubIDLists[nCIDX].IndexOf(DBHum.sAccount);
      if J < 0 then begin //T
        m_sCurMessage := '错误(保存人物失败):' + DBHum.sChrName + '  ID.DB无匹配账号:'+DBHum.sAccount;
        {$IF THREADWORK}Synchronize({$IFEND}OutMessage{$IF THREADWORK}){$IFEND};
        Inc(m_nFailed);
        Inc(m_nTotalPostion);
        Inc(m_nCurPostion);
