...
//没有用到 20071115  
  ...
  TnewHumData = packed record //新的数据结构
    sChrName: string[14];
    sCurMap: string[16];
    wCurX: Word;
    wCurY: Word;
    ...
    nReserved: Integer; //保留
    nReserved1: Integer; //保留
    nReserved2: Integer; //保留
    nReserved3: Integer; //保留
    n_Reserved: Word;   //保留
    n_Reserved1: Word;  //保留
    n_Reserved2: Word;  //保留
    n_Reserved3: Word;  //保留
    boReserved: Boolean; //保留
    boReserved1: Boolean;//保留
    boReserved2: Boolean;//保留
    boReserved3: Boolean;//保留
    sReserved:string[20];//保留
    sReserved1:string[20];//保留
  end;

  pTHumData = ^THumData;
  THumData = packed record //旧的数据结构
    sChrName: string[14];
    sCurMap: string[16];
    wCurX: Word;
    wCurY: Word;
  ...
  THumData1 = packed record //旧的数据结构--2
    sChrName: string[14];
    sCurMap: string[16];
    wCurX: Word;
    wCurY: Word;
...
const
  DBFileDesc = '清客网络数据库文件 2007/11/14';
