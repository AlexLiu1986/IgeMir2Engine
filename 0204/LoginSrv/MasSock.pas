...
begin
  try
    ...
    MainOutMessage('[异常] TFrmMasSoc.RefServerLimit');
  end;
end;
...
begin
  try
    ...
    MainOutMessage('[异常] TFrmMasSoc.SortServerList');
  end;
end;
...
begin
  try
    ...
    MainOutMessage('[异常] TFrmMasSoc.SendServerMsg');
  end;
end;
...
begin
  try
    ...
    MainOutMessage('[异常] TFrmMasSoc.GetOnlineHumCount');
  end;
end;
...
begin
  try
    ...
    on e: Exception do begin
      MainOutMessage('[异常] TFrmMasSoc.SendServerMsgA');
...
begin
  try
    ...
    MainOutMessage('[异常] TFrmMasSoc.LimitName');
  end;
end;
...
begin
  ...
  if FileExists(sFileName) then begin
    ...
    nUserLimit := nC;
    LoadList.Free;
  end else ShowMessage('[关键失败] 文件没有找到. .\!UserLimit.txt');
end;
