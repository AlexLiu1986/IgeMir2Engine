...
begin
  ...
  if boGateReady then begin
    ...
    if Socket.nIndex >= 0 then begin  //客户端连接网关  网关通知代理 在代理的list里加一个用户
    ...
    if (sReviceMsg <> '') and (boServerReady) then begin
      ...
      if nAttackLevel > 0 then begin
       ...
       { MainOutMessage('nMsgCount: ' + IntToStr(nMsgCount), 1);
        ...
        if nMsgLen > 962 * nAttackLevel then bo01 := True; //20080831 修改
...
begin
  try
    ...
    dwReConnectServerTick := GetTickCount - {25 * 1000}25000;
    boKeepAliveTimcOut := False;
    nSendMsgCount := 0;
    n456A2C := 0;
    dwSendKeepAliveTick := GetTickCount();
...
begin
  ...
  with frmGeneralConfig do begin
    ...
    SpinEditKeepConnectTime.Value := dwKeepConnectTimeOut div 1000;
  end;
