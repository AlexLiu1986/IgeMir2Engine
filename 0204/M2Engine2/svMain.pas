...
uses
  ...
  AboutUnit, HeroConfig, ViewList2, GuildManage;
...
begin
  try
    ...
    for I := Low(g_Config.GlobalVal) to High(g_Config.GlobalVal) do begin //保存系统变量
      Config.WriteInteger('Setup', 'GlobalVal' + IntToStr(I), g_Config.GlobalVal[I]);//20090101 修改
    end;
    for I := Low(g_Config.GlobalAVal) to High(g_Config.GlobalAVal) do begin//保存系统变量
      Config.WriteString('Setup', 'GlobalStrVal' + IntToStr(I), g_Config.GlobalAVal[I]);//20090101 修改
    end;
...
begin
  ...
  try
   ...
   { if (nGetSysDate >= 0) and Assigned(PlugProcArray[nGetSysDate].nProcAddr) then
    begin//检查系统插件是否为IGE插件 20081203
      if PlugProcArray[nGetSysDate].nProcCode = 9 then
      begin
