  ...
  if FileExists(sFileName) then begin
    ...
    for I := 0 to LoadList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if (MonGenInfo.sMapName <> '') and (MonGenInfo.sMonName <> '') and
...
//-----------------------------------------------------------------------------
    {if sCmd = sCHANGEMODE then begin//20090111 注释
  ...
  Try
    ...
    if UserEngine.QuestNPCList.Count > 0 then begin//20080629
      for I := 0 to UserEngine.QuestNPCList.Count - 1 do begin
        NPC := TNormNpc(UserEngine.QuestNPCList.Items[I]);                              //20090107 
    ...
    if UserEngine.m_MerchantList.Count > 0 then begin//20080629
      for I := 0 to UserEngine.m_MerchantList.Count - 1 do begin
        ...
        if Merchant <> nil then begin
          if (Merchant <> g_FunctionNPC) and (Merchant.m_boIsQuest) then//20090107 增加 m_boIsQuest 条件
          Merchant.m_nFlag := -1;
        end;
      end;
    end;
      ...
      try
        if sSellOffItemList.Count > 0 then begin//20080629
          for I:= 0 to sSellOffItemList.Count -1 do begin
            ...
            if DealOffInfo <> nil then FileWrite(FileHandle, DealOffInfo^, SizeOf(TDealOffInfo));//20090129 修改
          end;
        end;
      except
      end;
