...
type
  ...
  TUserCastle = class
    ...
    m_sName: string; //城堡名称
    m_sOwnGuild: string; //所属行会名称
    m_MasterGuild: TGUild; //所属行会
    m_sHomeMap: string; //行会回城点地图
    m_nHomeX: Integer; //行会回城点X
    m_nHomeY: Integer; //行会回城点Y
    ...
    //m_boStatus: Boolean;//未使用 20080702
  ...
  private
   ...
   // function m_nChiefItemCount: Integer; //未使用 20080330
  public
...
begin
  ...
  if CastleConf <> nil then begin
    ...
    if sMapList <> '' then begin
      if IsReLoad then m_EnvirList.Clear;//如果是重载，则清空列表内容，以防止重复 20081130
    ...
    //if m_MainDoor.nHP <= 0 then m_MainDoor.nHP := 2000;//20081028 修改
    ...
    //if m_LeftWall.nHP <= 0 then m_LeftWall.nHP := 2000;//20081028 修改
    ...
    //if m_CenterWall.nHP <= 0 then m_CenterWall.nHP := 2000;//20081028 修改
    if m_LeftWall.nHP <= 0 then begin
      if m_CenterWall.BaseObject <> nil then begin
    ...
    //if m_RightWall.nHP <= 0 then m_RightWall.nHP := 2000; //20081028 修改
...
begin
  ...
  try
    ...
    MainOutMessage('{异常} TUserCastle.SaveConfigFile Code:'+IntToStr(nCode));
  end;
end;
//读取攻击列表
...
resourcestring
  ...
  sExceptionMsg = '{异常} TUserCastle::Run';
begin
  try
...
{$IF SoftVersion <> VERDEMO}
    ...
    //MainOutMessage('m_LeftWall.BaseObject.m_boStoneMode:'+booltostr(m_LeftWall.BaseObject.m_boStoneMode));
    ...
    if m_boUnderWar then begin//正在攻城
      if (m_MainDoor.BaseObject <> nil) then begin//20081201 如果怪的名与设置不对应，则重建
        if (CompareText(m_MainDoor.BaseObject.m_sCharName, m_MainDoor.sName) <> 0) then begin
          ...
          m_MainDoor.BaseObject.m_Castle := self;//20090102
        end;
      end;
      if m_LeftWall.BaseObject <> nil then begin//20081201 如果怪的名与设置不对应，则重建
        if (CompareText(m_LeftWall.BaseObject.m_sCharName, m_LeftWall.sName) <> 0) then begin//20081201
          ...
          m_LeftWall.BaseObject.m_Castle := self;//20090102
        end;
        m_LeftWall.BaseObject.m_boStoneMode := False;
      end;
      if m_CenterWall.BaseObject <> nil then begin//20081201 如果怪的名与设置不对应，则重建
        if (CompareText(m_CenterWall.BaseObject.m_sCharName, m_CenterWall.sName) <> 0) then begin//20081201
          ...
          m_CenterWall.BaseObject.m_Castle := self;//20090102
        end;
        m_CenterWall.BaseObject.m_boStoneMode := False;
      end;
      if m_RightWall.BaseObject <> nil then begin//20081201 如果怪的名与设置不对应，则重建
        if (CompareText(m_RightWall.BaseObject.m_sCharName, m_RightWall.sName) <> 0) then begin//20081201
          ...
          m_RightWall.BaseObject.m_Castle := self;//20090102
        end;
...
begin
  ...
  try  
    ...
    MainOutMessage('{异常} TUserCastle.GetCastle Code:'+inttostr(nCode));
  end;
end;
        ...
        if m_MapCastle <> nil then begin
          if m_MainDoor.BaseObject <> nil then begin
            if (not m_MainDoor.BaseObject.m_boGhost) and (not m_MainDoor.BaseObject.m_boDeath)
              ...
              m_MainDoor.BaseObject.m_Castle := Castle;//20090102
              MainOutMessage('[城堡重载A-5] ' + m_MainDoor.BaseObject.m_sCharName);
            end else begin
              if m_MainDoor.BaseObject <> nil then begin
                if (CompareText(m_MainDoor.BaseObject.m_sCharName, m_MainDoor.sName) = 0) then begin
                  if not m_MainDoor.BaseObject.m_boDeath then begin
                    ...
                    m_MainDoor.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载A-1] ' + m_MainDoor.BaseObject.m_sCharName);
                  end else begin
                    m_MainDoor.BaseObject.m_WAbil.HP := m_MainDoor.BaseObject.m_WAbil.MaxHP;
                    TCastleDoor(m_MainDoor.BaseObject).m_boOpened := boUnderWar;//城门是否关闭
                    m_MainDoor.BaseObject.ReAlive;//20081202
                    m_MainDoor.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载A-2] ' + m_MainDoor.BaseObject.m_sCharName);
                  end;
                end else begin
                  m_MainDoor.BaseObject := UserEngine.RegenMonsterByName(m_sMapName, m_MainDoor.nX, m_MainDoor.nY, m_MainDoor.sName);
                  if m_MainDoor.BaseObject <> nil then begin
                    ...
                    m_MainDoor.BaseObject.m_nCurrX := m_MainDoor.nX;
                    m_MainDoor.BaseObject.m_nCurrY := m_MainDoor.nY;
                    MainOutMessage('[城堡重载A-3] ' + m_MainDoor.BaseObject.m_sCharName);
                  end else begin              
                    MainOutMessage('[错误信息] 城堡初始化城门失败，检查怪物数据库里有没城门的设置: ' + m_MainDoor.sName);
                  end;
                end;
                ...
                if m_MainDoor.BaseObject <> nil then begin
                  m_MainDoor.BaseObject.m_WAbil.HP := m_MainDoor.nHP;
                  m_MainDoor.BaseObject.m_Castle := Castle;
                  MainOutMessage('[城堡重载A-4] ' + m_MainDoor.BaseObject.m_sCharName);
                end else begin
                  MainOutMessage('[错误信息] 城堡初始化城门失败，检查怪物数据库里有没城门的设置: ' + m_MainDoor.sName);
                end;
              end;
          ...
          if m_LeftWall.BaseObject <> nil then begin
            if (not m_LeftWall.BaseObject.m_boGhost) and (not m_LeftWall.BaseObject.m_boDeath)
              ...
              MainOutMessage('[城堡重载B-5] ' + m_LeftWall.BaseObject.m_sCharName);
            end else begin
              if m_LeftWall.BaseObject <> nil then begin
                if (CompareText(m_LeftWall.BaseObject.m_sCharName, m_LeftWall.sName) = 0) then begin
                  if not m_LeftWall.BaseObject.m_boDeath then begin
                    ...
                    m_LeftWall.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载B-1] ' + m_LeftWall.BaseObject.m_sCharName);
                  end else begin
                    m_LeftWall.BaseObject.m_WAbil.HP := m_LeftWall.BaseObject.m_WAbil.MaxHP;
                    m_LeftWall.BaseObject.ReAlive;//20081202
                    m_LeftWall.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载B-2] ' + m_LeftWall.BaseObject.m_sCharName);
                  end;
                end else begin
                  m_LeftWall.BaseObject:= UserEngine.RegenMonsterByName(m_sMapName, m_LeftWall.nX, m_LeftWall.nY, m_LeftWall.sName);
                  if m_LeftWall.BaseObject <> nil then begin
                    ...
                    m_LeftWall.BaseObject.m_nCurrX := m_LeftWall.nX;
                    m_LeftWall.BaseObject.m_nCurrY := m_LeftWall.nY;
                    MainOutMessage('[城堡重载B-3] ' + m_LeftWall.BaseObject.m_sCharName);
                  end else begin
                    MainOutMessage('[错误信息] 城堡初始化左城墙失败，检查怪物数据库里有没左城墙的设置: ' + m_LeftWall.sName);
                  end;
                end;
                ...
                if m_LeftWall.BaseObject <> nil then begin
                  m_LeftWall.BaseObject.m_WAbil.HP := m_LeftWall.nHP;
                  m_LeftWall.BaseObject.m_Castle := Castle;
                  MainOutMessage('[城堡重载B-4] ' + m_LeftWall.BaseObject.m_sCharName);
                end else begin
                  MainOutMessage('[错误信息] 城堡初始化左城墙失败，检查怪物数据库里有没左城墙的设置: ' + m_LeftWall.sName);
                end;
              end;
            ...
            MainOutMessage('[错误信息] 城堡初化左城墙失败，检查怪物数据库里有没左城墙的设置: ' + m_LeftWall.sName);
          end;
          ...
          if m_CenterWall.BaseObject <> nil then begin
            if (not m_CenterWall.BaseObject.m_boGhost) and (not m_CenterWall.BaseObject.m_boDeath)
              ...
              MainOutMessage('[城堡重载C-5] ' + m_CenterWall.BaseObject.m_sCharName);
            end else begin
              if m_CenterWall.BaseObject <> nil then begin
                if (CompareText(m_CenterWall.BaseObject.m_sCharName, m_CenterWall.sName) = 0) then begin
                  if not m_CenterWall.BaseObject.m_boDeath then begin
                    ...
                    m_CenterWall.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载C-1] ' + m_CenterWall.BaseObject.m_sCharName);
                  end else begin
                    m_CenterWall.BaseObject.m_WAbil.HP := m_CenterWall.BaseObject.m_WAbil.MaxHP;
                    m_CenterWall.BaseObject.ReAlive;//20081202
                    m_CenterWall.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载C-2] ' + m_CenterWall.BaseObject.m_sCharName);
                  end;
                end else begin
                  m_CenterWall.BaseObject := UserEngine.RegenMonsterByName(m_sMapName, m_CenterWall.nX, m_CenterWall.nY, m_CenterWall.sName);
                  if m_CenterWall.BaseObject <> nil then begin
                    ...
                    m_CenterWall.BaseObject.m_nCurrX := m_CenterWall.nX;
                    m_CenterWall.BaseObject.m_nCurrY := m_CenterWall.nY;
                    MainOutMessage('[城堡重载C-3] ' + m_CenterWall.BaseObject.m_sCharName);
                  end else begin
                    MainOutMessage('[错误信息] 城堡初始化中城墙失败，检查怪物数据库里有没中城墙的设置: ' + m_CenterWall.sName);
                  end;
                end;
                ...
                if m_CenterWall.BaseObject <> nil then begin
                  m_CenterWall.BaseObject.m_WAbil.HP := m_CenterWall.nHP;
                  m_CenterWall.BaseObject.m_Castle := Castle;
                  MainOutMessage('[城堡重载C-4] ' + m_CenterWall.BaseObject.m_sCharName);
                end else begin
                  MainOutMessage('[错误信息] 城堡初始化中城墙失败，检查怪物数据库里有没中城墙的设置: ' + m_CenterWall.sName);
                end;
              end;
          ...
          if m_RightWall.BaseObject <> nil then begin
            if (not m_RightWall.BaseObject.m_boGhost) and (not m_RightWall.BaseObject.m_boDeath)
              ...
              MainOutMessage('[城堡重载D-5] ' + m_RightWall.BaseObject.m_sCharName);
            end else begin
              if m_RightWall.BaseObject <> nil then begin
                if (CompareText(m_RightWall.BaseObject.m_sCharName, m_RightWall.sName) = 0) then begin
                  if not m_RightWall.BaseObject.m_boDeath then begin
                    ...
                    m_RightWall.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载D-1] ' + m_RightWall.BaseObject.m_sCharName);
                  end else begin
                    m_RightWall.BaseObject.m_WAbil.HP := m_RightWall.BaseObject.m_WAbil.MaxHP;
                    m_RightWall.BaseObject.ReAlive;//20081202
                    m_RightWall.BaseObject.m_Castle := Castle;//20090102
                    MainOutMessage('[城堡重载D-2] ' + m_RightWall.BaseObject.m_sCharName);
                  end;
                end else begin
                  m_RightWall.BaseObject := UserEngine.RegenMonsterByName(m_sMapName, m_RightWall.nX, m_RightWall.nY, m_RightWall.sName);
                  if m_RightWall.BaseObject <> nil then begin
                    ...
                    m_RightWall.BaseObject.m_nCurrX := m_RightWall.nX;
                    m_RightWall.BaseObject.m_nCurrY := m_RightWall.nY;
                    MainOutMessage('[城堡重载D-3] ' + m_RightWall.BaseObject.m_sCharName);
                  end else begin
                    MainOutMessage('[错误信息] 城堡初始化右城墙失败，检查怪物数据库里有没右城墙的设置: ' + m_RightWall.sName);
                  end;
                end;
                ...
                if m_RightWall.BaseObject <> nil then begin
                  m_RightWall.BaseObject.m_WAbil.HP := m_RightWall.nHP;
                  m_RightWall.BaseObject.m_Castle := Castle;
                  MainOutMessage('[城堡重载D-4] ' + m_RightWall.BaseObject.m_sCharName);
                end else begin
                  MainOutMessage('[错误信息] 城堡初始化右城墙失败，检查怪物数据库里有没右城墙的设置: ' + m_RightWall.sName);
                end;
              end;
...
begin
  ...
  try
    Try
      ...
      MainOutMessage('{异常} TCastleManager.Run');
    end;
  finally
    UnLock;
  end;
...
begin
  Try
    ...
    MainOutMessage('{异常} CastleManager.SaveCastleList...'+ g_Config.sCastleFile);
  end;
end;
