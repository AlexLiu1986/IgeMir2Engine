...
type
  THeroObject = class(TBaseObject)
   ...
   // m_nStartUseSpell: Integer;
    ...
    m_GetExp: LongWord;//英雄取得的经验,$HeroGetExp变量使用  20081228
...
begin
  try
    ...
    MainOutMessage('{异常} THeroObject.SendUseitems');
  end;
end;
...
begin
  ...
  try
    ...
    m_GetExp:= dwExp;//英雄取得的经验,$HeroGetExp变量使用  20081228
    ...
    if (m_Master <> nil) then begin//20090102 //只发送给非离线挂机人物
      if not TPlayObject(m_Master).m_boNotOnlineAddExp then SendMsg(m_Master, RM_HEROWINEXP, 0, dwExp, 0, 0, '');
    end;
...
begin
  try
    ...
    MainOutMessage('{异常} THeroObject.Operate');
  end;
end;
{ 20080117
  ...
  case m_btJob of
    0: begin //战士
       ...
       //远距离则用开天重击或是逐日剑法 20080603   20090115 加入=
            ...
            case Random(3) of
              0:begin                                                                                                                                    //20080710 PK时不用狮子吼
                  if IsAllowUseMagic(41) and (GetTickCount - m_SkillUseTick[41] >  10000{10 * 1000})
                    and (m_TargetCret.m_Abil.Level < m_Abil.Level) and (m_TargetCret.m_btRaceServer <> RC_PLAYOBJECT)
                    and (m_TargetCret.m_btRaceServer <> RC_HEROOBJECT) and (abs(m_TargetCret.m_nCurrX - m_nCurrX) <= 3) and (abs(m_TargetCret.m_nCurrY - m_nCurrY) <= 3) then begin
        ...
        if IsAllowUseMagic(41) and (GetTickCount - m_SkillUseTick[41] > 10000)
          and (m_TargetCret.m_Abil.Level < m_Abil.Level) and (m_TargetCret.m_btRaceServer <> RC_PLAYOBJECT)
          and (m_TargetCret.m_btRaceServer <> RC_HEROOBJECT) and (abs(m_TargetCret.m_nCurrX - m_nCurrX) <= 3) and (abs(m_TargetCret.m_nCurrY - m_nCurrY) <= 3) then begin//狮子吼
  ...
  (* //PK 时,先无极后双毒,打怪时是先双毒再无极 20080711
    if ((m_TargetCret.m_btRaceServer = RC_PLAYOBJECT) or (m_TargetCret.m_btRaceServer = RC_HEROOBJECT) or (m_TargetCret.m_Master <> nil)) then begin//PK
       ...
       if IsAllowUseMagic(50) and (GetTickCount - m_SkillUseTick[50] > g_Config.nAbilityUpTick * 1000) and (m_wStatusArrValue[2]=0)
         and ((g_Config.btHeroSkillMode50) or (m_TargetCret.m_Abil.HP >=700) or ((m_TargetCret.m_btRaceServer = RC_PLAYOBJECT) or (m_TargetCret.m_btRaceServer = RC_HEROOBJECT) or (m_TargetCret.m_btRaceServer = RC_PLAYMOSTER)))then begin//20080827
          m_SkillUseTick[50] := GetTickCount;
          Result := 50;
          Exit;
        end;
      if (m_TargetCret.m_wStatusTimeArr[POISON_DECHEALTH] = 0) and (GetUserItemList(2,1)>= 0) //绿毒
       and ((g_Config.btHeroSkillMode) or (m_TargetCret.m_Abil.HP >=700) or ((m_TargetCret.m_btRaceServer = RC_PLAYOBJECT) or (m_TargetCret.m_btRaceServer = RC_HEROOBJECT) or (m_TargetCret.m_btRaceServer = RC_PLAYMOSTER)) or m_boTarget)
      ...
      if (m_TargetCret.m_wStatusTimeArr[POISON_DECHEALTH] = 0) and (GetUserItemList(2,1)>= 0) //绿毒
        ...
        and (m_TargetCret.m_btRaceServer <> 110) and (m_TargetCret.m_btRaceServer <> 111) and (m_TargetCret.m_btRaceServer <> 55) and (m_TargetCret.m_btRaceServer <> 128) then begin //对于血量超过800的怪用  修改距离 20080704 不毒城墙
        n_AmuletIndx:= 0;//20080413
        case Random(2) of
         0: begin
              if IsAllowUseMagic(38) and (GetTickCount - m_SkillUseTick[38] > 1000) then  begin
      ...
      if (m_TargetCret.m_wStatusTimeArr[POISON_DAMAGEARMOR] = 0) and (GetUserItemList(2,2)>= 0) //红毒
        and ((g_Config.btHeroSkillMode) or (m_TargetCret.m_Abil.HP >= 700) or ((m_TargetCret.m_btRaceServer = RC_PLAYOBJECT) or (m_TargetCret.m_btRaceServer = RC_HEROOBJECT) or (m_TargetCret.m_btRaceServer = RC_PLAYMOSTER)) or m_boTarget)
        and ((abs(m_TargetCret.m_nCurrX - m_nCurrX) < 7{4}) or (abs(m_TargetCret.m_nCurrY - m_nCurrY) < 7{4}))
        and (m_TargetCret.m_btRaceServer <> 110) and (m_TargetCret.m_btRaceServer <> 111) and (m_TargetCret.m_btRaceServer <> 55) and (m_TargetCret.m_btRaceServer <> 128) then begin //对于血量超过100的怪用 不毒城墙
        n_AmuletIndx:= 0;//20080413
        case Random(2) of
         0: begin
              if IsAllowUseMagic(38) and (GetTickCount - m_SkillUseTick[38] > 1000) then  begin
...
begin
  ...
  if (m_TargetCret <> nil) and (GetTickCount - m_dwAutoAvoidTick > 1100) and ((not m_boIsUseAttackMagic) or (m_btJob = 0)) then begin
    if m_btJob > 0 then begin
      ...
      if (m_btStatus <> 2) and ((g_Config.boHeroAttackTarget and (m_Abil.Level < 22)) or   //20090108
         ...
         (m_btJob = 2) and ((abs(m_TargetCret.m_nCurrX - m_nCurrX) > 1) or (abs(m_TargetCret.m_nCurrY - m_nCurrY) > 1)))) then begin//20081218 道法22前是否物理攻击
        Result := True;
        Exit;      
      end;
    end else
...
begin
  ...
  if (g_Config.boHeroAttackTarget and (m_Abil.Level < 22)) or
    ((m_TargetCret.m_WAbil.MaxHP < 700) and g_Config.boHeroAttackTao and (m_TargetCret.m_btRaceServer <> RC_PLAYOBJECT) and (m_TargetCret.m_btRaceServer <> RC_HEROOBJECT)) then begin//20090106 道士22级前是否物理攻击  怪等级小于英雄时
    m_boIsUseMagic := False;//是否能躲避
    Result := WarrAttackTarget(m_wHitMode);
  end;
end;
                  ...
                  case GetTogetherUseSpell of
                     60:begin
                        if ((m_wStatusTimeArr[POISON_STONE] = 0) and (m_Master.m_wStatusTimeArr[POISON_STONE] = 0))
                          ...
                          end; } //20090105修改，直线才放破魂斩
                          if (((abs(m_TargetCret.m_nCurrX - m_nCurrX) = 2) and (abs(m_TargetCret.m_nCurrY - m_nCurrY) = 0)) or
                              ...
                              ((abs(m_TargetCret.m_nCurrX - m_nCurrX) = 0) and (abs(m_TargetCret.m_nCurrY - m_nCurrY) = 2))) and
                             (((abs(m_TargetCret.m_nCurrX - m_Master.m_nCurrX) = 2) and (abs(m_TargetCret.m_nCurrY - m_Master.m_nCurrY) = 0)) or
                              ...
                              ((abs(m_TargetCret.m_nCurrX - m_Master.m_nCurrX) = 0) and (abs(m_TargetCret.m_nCurrY - m_Master.m_nCurrY) = 2))) then begin
                             m_boDecDragonPoint:= False;//停止减怒气
                             m_boStartUseSpell := True;//合击技能可用
                             m_dwStartUseSpellTick := GetTickCount();
                          end;
...
begin
 inherited;
   ...
   if g_Config.boHeroDieExp then begin
     nDecExp:= Round({GetLevelExp(m_Abil.Level)}m_Abil.Exp * (g_Config.nHeroDieExpRate / 100));//掉现有经验的一定比率 20090108
     SysMsg('(英雄) 死亡，其经验值减少了 '+IntToStr(nDecExp), c_Red, t_Hint); //20090108
     if m_Abil.Exp >= nDecExp then begin
        Dec(m_Abil.Exp, nDecExp);
     end else begin
       m_Abil.Exp := 0;//20090108 掉完经验不降级
...
begin
  ...
  try
    ...
    on E: Exception do begin
      MainOutMessage(Format('{异常} THeroObject.DoSpell MagID:%d X:%d Y:%d Code:%d', [UserMagic.wMagIdx, nTargetX, nTargetY, nCode]));
    end;
  end;
end;
