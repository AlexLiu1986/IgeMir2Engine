...
begin
  try
    ...
    MainOutMessage('{异常} TGuildManager.Run');
  end;
end;
...
begin
  ...
  try
    ...
    MainOutMessage('{异常} TGUild.RefMemberName Code:'+inttostr(nCode));
  end;
end;
...
begin
  ...
  try
        ...
        if GuildRank.MemberList.Count > 0 then begin//20080630
          for II := 0 to GuildRank.MemberList.Count - 1 do begin
            ...
            if PlayObject.m_boBanGuildChat then begin
              ...
              nCheckCode := 6;
              if g_Config.boRecordClientMsg and (sMsg <> '') then begin//记录行会聊天信息 20090129
                MainOutMessage('[行会聊天] '+ sGuildName+':'+ sMsg);
              end;
            end;
          end;//for
        end;
    ...
    on E: Exception do begin
      MainOutMessage('{异常} TGuild.SendGuildMsg CheckCode: ' + IntToStr(nCheckCode) + ' GuildName:' + sGuildName + ' Msg:' + sMsg);
...
begin
  ...
  try
    ...
    on E: Exception do begin
      MainOutMessage('{异常} TGuild.SendGuildMsg1 CheckCode: ' + IntToStr(nCheckCode) + ' GuildName:' + sGuildName + ' Msg:' + sMsg);
      //MainOutMessage(E.Message);
    end;
  end;
end;
...
begin
  ...
  Try
    ...
    MainOutMessage('{异常} TGUild.UpdateRank');
  end;
end;
