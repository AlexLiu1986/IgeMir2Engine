...
begin
  try
    ...
    MainOutMessage('{异常} TPlayMonster.Operate');
  end;
end;
...
begin
  ...
  try
    if (not m_boGhost) and (not m_boDeath) and (not m_boFixedHideMode) and (not m_boStoneMode) and
      ...
      nCheckCode:= 2;
      if (GetTickCount - m_dwSearchEnemyTick) > 1100 then begin//20090104 1000改成1100
        m_dwSearchEnemyTick := GetTickCount();
...
begin
  Try
    ...
    MainOutMessage('{异常} TPlayMonster.PlaySuperRock');
  end;
end;
