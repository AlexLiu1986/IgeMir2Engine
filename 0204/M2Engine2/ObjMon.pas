...
begin
  Result := False;                       //20090105
  if (*(GetTickCount - m_dwAutoAvoidTick > {1000}450) and*) (m_TargetCret <> nil) and (not m_boIsUseAttackMagic) then begin //20080618 怪在近身二格内
  ...
  Try
    if (not m_boDeath) and (not m_boGhost) and (m_wStatusTimeArr[POISON_STONE] = 0) then begin
      ...
      if ({Integer}(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin//走路间隔
        ...
        if (m_Master<> nil) then begin//20090202 主人休息状态时的控制
    ...
    MainOutMessage('{异常} TFairyMonster.Run Code:'+inttostr(nCode));
  end;
  inherited;
end;
...
begin
  ...
  Try
    ...
    MainOutMessage('{异常} TSwordsmanMon.Run Code:'+inttostr(nCode));
  end;
  inherited;
end;
